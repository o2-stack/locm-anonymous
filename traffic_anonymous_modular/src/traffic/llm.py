...
class BasinLLMAnalyzer:
    SYSTEM_PROMPT = (
        ...
        "You analyze behavioral trajectories of a multi-intersection traffic signal control system.\n"
        "Each agent controls the green split ratio at one intersection in a corridor network.\n"
        "Agents use DISCRETE actions: keep(0), +5%(1), -5%(2), +10%(3), -10%(4).\n"
        "Do NOT directly turn traffic metrics (delay, queue length) into reward terms.\n"
        "However, you MUST treat traffic safety and efficiency as hard constraints.\n"
        "Any recommended coordination basin must remain within acceptable traffic performance bounds.\n"
        "If the current behavior becomes more stable but traffic outcomes become clearly worse, "
    ...
    def round_review(self, phase_summaries: List[PhaseSummary],
                     knowledge: BasinKnowledgeBase = None) -> dict:
        if not phase_summaries:
            review = {
                "n_phases": 0, "round_stage": "unknown",
        ...
        if not overviews:
            review = {
                "n_phases": len(phase_summaries), "round_stage": "unknown",
        ...
        avg_delay = float(np.mean([o.get("avg_delay", 0.0) for o in overviews]))
        ...
        last_delay = float(last_overview.get("avg_delay", 0.0))

        first_overview = overviews[0]
        reward_change = last_reward - float(first_overview.get("avg_reward", 0.0))
        delay_change = last_delay - float(first_overview.get("avg_delay", 0.0))
        ...
        review = {
            ...
            "delay_change_first_to_last_phase": round(delay_change, 4),
    ...
    def reflect_round(self, phase_summaries: List[PhaseSummary],
                      knowledge: BasinKnowledgeBase = None) -> dict:
        if not phase_summaries:
            reflection = {
                ...
                "search_bias_update": {"exploration": 0.6, "verification": 0.3, "exploitation": 0.1},
        ...
        if not overviews:
            reflection = {
                ...
                "search_bias_update": {"exploration": 0.5, "verification": 0.4, "exploitation": 0.1},
        ...
        if avg_dom > 0.5:
            ...
            what_not_to_repeat.append("do not over-concentrate control burden on a single intersection")
        ...
        if avg_delay > 0.4:
            failure_modes.append("behaviorally_structured_but_traffic_inefficient")
            what_not_to_repeat.append("do not continue strengthening basins that look coordinated but correlate with poor traffic outcomes")
            next_directions.append("explore alternative coordination structures that may align better with traffic efficiency")
        ...
        elif avg_delay > 0.4 and avg_counter < 0.4:
            bad_basin = "stable_but_traffic_misaligned_basin"
        elif avg_dom > 0.5:
            bad_basin = "single_intersection_dominance_basin"
        elif avg_burden > 0.1:
            bad_basin = "high_maintenance_basin"
        else:
            bad_basin = "uncertain_or_mixed_basin"

        if "behaviorally_structured_but_traffic_inefficient" in failure_modes:
            search_bias = {"exploration": 0.6, "verification": 0.3, "exploitation": 0.1}
        elif len(failure_modes) >= 3:
            search_bias = {"exploration": 0.7, "verification": 0.2, "exploitation": 0.1}
        else:
            search_bias = {"exploration": 0.4, "verification": 0.4, "exploitation": 0.2}
    ...
    def _rule_based_analysis(self, summary: PhaseSummary) -> dict:
        ...
        avg_max_queue = float(o.get("avg_max_queue", 0.0))
        avg_throughput = float(o.get("avg_throughput", 0.0))
        ...
        portrait = {
            ...
            "key_observations": [
                ...
                f"avg_delay={avg_delay:.3f}",
                f"avg_max_queue={avg_max_queue:.3f}",
                f"avg_throughput={avg_throughput:.3f}",
                f"avg_overflow={avg_overflow:.3f}",
            ],
            ...
            "good_patterns_to_strengthen": [
                "more stable signal coordination over time",
                "lower action opposition across intersections",
            ],
            "bad_patterns_to_suppress": [
                "high counteraction between neighboring intersections",
                "growing control burden",
            ],
            "uncertainty_areas": [
                "whether recent coordination is genuine or only superficial suppression"
            ],
            "reasoning": "Rule-based fallback portrait for discrete traffic signal coordination."
        }
        return portrait
